...
class StringAutoEditStrategyTest extends AutoEditStrategyTests {
  ...
  val strategy = new StringAutoEditStrategy(ScalaPartitions.ScalaPartitioning, prefStore)

  @Before
  def startUp() {
    enable(EnableAutoEscapeLiterals, true)
  ...
  def no_auto_escape_string_literal_if_feature_deactivated() {
    enable(EnableAutoEscapeLiterals, false)
    test(input = """ "^ " """, expectedOutput = """ ""^ " """, operation = Add("\""))
  }
