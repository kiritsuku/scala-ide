...
object SDTTestUtils {
  ...
  def sourceWorkspaceLoc(containingBundleName: String) = {
    val bundle = Platform.getBundle(containingBundleName)
   ...
   */
  def setupProject(name: String, containingBundleName: String): ScalaProject = {
    EclipseUtils.workspaceRunnableIn(workspace) { monitor =>
      val wspaceLoc = workspace.getRoot.getLocation
      val src = new File(sourceWorkspaceLoc(containingBundleName).toFile().getAbsolutePath + File.separatorChar + name)
