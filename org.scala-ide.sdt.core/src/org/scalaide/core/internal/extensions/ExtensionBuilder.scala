...
object ExtensionBuilder extends HasLogger {
  ...
  private val tbf = new ToolBoxFactory(universe) {
    val mirror = u.runtimeMirror(cl)
  }
  private[extensions] val toolBox = tbf.mkToolBox()
...
//            }
  ...
  def createSaveAction(ext: String): Seq[ExtensionBuilder#Creator] = {
    ...
    util.Try {compile(untypecheck(typecheck(extension)))} foreach (x => println(">>>>>>>>"+x))
    val exts =
...
trait ExtensionBuilder {
  import ExtensionBuilder.toolBox._, u._
