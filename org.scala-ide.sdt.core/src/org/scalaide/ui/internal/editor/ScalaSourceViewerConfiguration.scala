...
class ScalaSourceViewerConfiguration(
  ...
  /**
   * This method seems to exist only for historical reasons. It doesn't seem to
   * be necessary to overwrite it.
   */
  override def getIndentPrefixesForTab(tabWidth: Int): Array[String] =
    throw new IllegalArgumentException(
        "This method should not be called. " +
        "Use `getIndentPrefixes(ISourceViewer, String)` instead. " +
        "If it turns out that this is not possible, then move all necessary parts of its implementation to this method.")
