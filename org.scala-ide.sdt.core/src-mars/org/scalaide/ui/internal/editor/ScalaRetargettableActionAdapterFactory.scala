...
class ScalaRetargettableActionAdapterFactory extends IAdapterFactory {
  override def getAdapter[A](adaptableObject : AnyRef, adapterType : Class[A]) =
    if (adapterType == classOf[IRunToLineTarget])
      new RunToLineAdapter().asInstanceOf[A]
    else if (adapterType == classOf[IToggleBreakpointsTarget])
      new ScalaToggleBreakpointAdapter().asInstanceOf[A]
    else
      null.asInstanceOf[A]
